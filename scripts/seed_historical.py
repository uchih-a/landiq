...
def prepare_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Drop rows with inf values (e.g. price_per_acre when size_acres == 0)
    import numpy as np
    numeric_cols = df.select_dtypes(include=[float, int]).columns
    before = len(df)
    df = df[~df[numeric_cols].isin([np.inf, -np.inf]).any(axis=1)]
    dropped_inf = before - len(df)
    if dropped_inf:
        print(f"Dropped {dropped_inf} rows with inf values ({before} -> {len(df)} rows)")
